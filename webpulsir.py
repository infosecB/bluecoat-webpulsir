...
class WebPulsIR(object):
    ...
    def reviewurl(self, url):
        classification = self.session.post('https://sitereview.bluecoat.com/resource/lookup', json={'captcha': '', 'url': url})
        parsedclassification = json.loads(classification.content.decode("UTF-8"))
        reviewresult = "Results from WebPulse \n\n"
        reviewresult += "Category: " + parsedclassification["categorization"][0]["name"] + "\n"
        reviewresult += "Date Reviewed: " + parsedclassification["translatedRateDates"][0]["text"][88:115] + "\n"
        reviewresult += "URL: " + parsedclassification["url"] + "\n"
        return reviewresult


    def submiturl(self, url):
        self.session.post('https://sitereview.bluecoat.com/resource/lookup', json={'captcha': '', 'url': url})
...
if __name__ == "__main__":
    ...
    argobj.add_argument("-r", action='store_true', help="Switch to specify review of URL to WebPulse.")
    argobj.add_argument("-s", action='store_true', help="Switch to specify submission of URL to WebPulse.")
